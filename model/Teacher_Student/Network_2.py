...
from model.Teacher_Student.Spatial_Channel_Attention import SpatialAttention,ChannelwiseAttention
...
class regionattention(nn.Module):
    def __init__(self, in_channel):
        ...
        self.ASPP_att=ASPPModule(in_channel,in_channel*3)

        #Channel and Spatial Attention
        self.ca=ChannelwiseAttention(in_channel*3)
        self.sa=SpatialAttention(in_channel*3)
    ...
    def forward(self,lung_flow,infection_flow):
        ...
        att_infection_fusion=self.ASPP_att(att_infection_flow)

        att_infection_fusion_casa=self.ca(self.sa(att_infection_fusion))

        att_infection=self.fusionconv(att_infection_fusion_casa)
...
class dualstream_TS_ASPP(nn.Module):
    def __init__(self,num_cls=3):
        super(dualstream_TS_ASPP, self).__init__()
        ...
        self.decoder4_infection = nn.Conv2d(96, num_cls, kernel_size=1, stride=1, padding=0)
        ...
        self.ms1_infection = nn.Conv2d(384, num_cls, kernel_size=1, stride=1, padding=0)
        ...
        self.ms2_infection = nn.Conv2d(192, num_cls, kernel_size=1, stride=1, padding=0)
        ...
        self.ms3_infection = nn.Conv2d(96, num_cls, kernel_size=1, stride=1, padding=0)
