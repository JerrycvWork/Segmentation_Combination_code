...
class decode_noatt_nolung_module(nn.Module):
    def __init__(self,in_channel,out_channel):
        super(decode_noatt_nolung_module, self).__init__()
    ...
    def forward(self,lung_flow,infection_flow):
        lung_flow2=self.regionconv1(lung_flow)
        infection_flow2=self.infconv1(infection_flow)
        lung_flow_replace=torch.ones(lung_flow2.shape).cuda()
        att_infection_flow=self.att_module(lung_flow_replace,infection_flow2)

        return lung_flow2,att_infection_flow



class dualstream_noatt_nolung(nn.Module):
    def __init__(self):
        super(dualstream_noatt_nolung, self).__init__()
        ...
        self.decode1=decode_noatt_nolung_module(768,384)
        self.up1=nn.UpsamplingBilinear2d(scale_factor=2)

        self.decode1_1 = decode_noatt_nolung_module(768, 384)

        self.decode2=decode_noatt_nolung_module(384,192)
        self.up2=nn.UpsamplingBilinear2d(scale_factor=2)

        self.decode2_1=decode_noatt_nolung_module(384,192)

        self.decode3 = decode_noatt_nolung_module(192, 96)
        self.up3 = nn.UpsamplingBilinear2d(scale_factor=2)

        self.decode3_1 = decode_noatt_nolung_module(192, 96)
        self.up4 = nn.UpsamplingBilinear2d(scale_factor=4)
    ...
    def forward(self,x):
        ...
        return infection_flow4,ms1_infection,ms2_infection,ms3_infection
...
class dualstream_noatt(nn.Module):
    def __init__(self):
        super(dualstream_noatt, self).__init__()
