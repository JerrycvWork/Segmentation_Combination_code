...
class decode_module(nn.Module):
    ...
    def forward(self,lung_flow,infection_flow,Prob_score):
        ...
        Prob_score = Prob_score.unsqueeze(1)
        new_lung_flow=torch.mul(lung_flow2,Prob_score)
...
class adv_dualstream(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(prob_score)
        prob_score=F.sigmoid(prob_score)
...
class dualstream_score(nn.Module):
    def __init__(self):
        super(dualstream_score, self).__init__()
    ...
    def forward(self,x):
        ...
        final_feat = self.up(final_feat)
        lung_flow1,infection_flow1,att1=self.decode1(final_feat,final_feat,prob_score)
        lung_flow1=self.up1(lung_flow1)
        infection_flow1 = self.up1(infection_flow1)

        lung_flow1_1, infection_flow1_1,att1_1= self.decode1_1(lung_flow1,infection_flow1,prob_score)
        lung_flow1_1 = self.up1_1(lung_flow1_1)
        infection_flow1_1 = self.up1_1(infection_flow1_1)

        lung_flow2, infection_flow2,att2= self.decode2(lung_flow1_1, infection_flow1_1,prob_score)
        lung_flow2 = self.up2(lung_flow2)
        infection_flow2 = self.up2(infection_flow2)

        lung_flow2_1, infection_flow2_1,att2_1= self.decode2_1(lung_flow2, infection_flow2,prob_score)
        lung_flow2_1 = self.up2_1(lung_flow2_1)
        infection_flow2_1 = self.up2_1(infection_flow2_1)

        lung_flow3, infection_flow3,att3= self.decode3(lung_flow2_1, infection_flow2_1,prob_score)
        ...
        att1=F.interpolate(att1,size=(512,512))
        att1_1 = F.interpolate(att1_1, size=(512, 512))
        att2 = F.interpolate(att2, size=(512, 512))
        att2_1 = F.interpolate(att2_1, size=(512, 512))
        att3 = F.interpolate(att3, size=(512, 512))

        return prob_score,lung_flow4,infection_flow4,ms1_lung,ms1_infection,ms2_lung,ms2_infection,[att1,att1_1,att2,att2_1,att3]
